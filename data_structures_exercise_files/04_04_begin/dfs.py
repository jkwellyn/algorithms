...
def dfs(maze, start, goal):    
    ...
    stack.push(start)
    # track the predescessors, your start should have no predessecor
    predecessors = {start: None}

    while not stack.is_empty():
        current_cell = stack.pop()
        if current_cell == goal:
            # if the current cell is the one you want, return the path it took to get there
            return get_path(predecessors, start, goal)
    
        # start adding some neigbors through offsets
        for direction in ["up", "right", "down", "left"]:
            row_offset, col_offset = offsets[direction] #gets the offset
            ...
            if is_legal_pos(maze, neighbor) and neighbor not in predecessors:
                # if the position is a valid position, add it to the stack
                stack.push(neighbor)
